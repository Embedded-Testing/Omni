...
class TestSalea(unittest.TestCase):
    ...
    def test_launch_salea_raises_exception_if_process_file_dont_exist(self):
        with self.assertRaises(FileNotFoundError):
            launch_salea(salea_path, salea_log_path,
                         dummy_port, DummyProcess_file)
    ...
    @patch('subprocess.Popen', side_effect=None)
    def test_launches_salea_application_correct_call(self, mock_subprocess):
        create_config_file(DummyProcess_file)
        process_file = os.path.abspath(DummyProcess_file)
        salea_launch_command = str(
            salea_path)+" "+"--automation --automationPort "+str(dummy_port)
        launch_salea(salea_path, salea_log_path, dummy_port, process_file)
        mock_subprocess.assert_called_with(
            salea_launch_command.split(), stdout=ANY, stderr=ANY)
        os.remove(DummyProcess_file)

    @patch('subprocess.Popen', side_effect=None)
    def test_launches_headless_salea_application_correct_call(self, mock_subprocess):
        create_config_file(DummyProcess_file)
        process_file = os.path.abspath(DummyProcess_file)
        salea_launch_command = "xvfb-run "+str(
            salea_path)+" "+"--automation --automationPort "+str(dummy_port)
        launch_salea(salea_path, salea_log_path, dummy_port,
                     process_file, headless=True)
        mock_subprocess.assert_called_with(
            salea_launch_command.split(), stdout=ANY, stderr=ANY)
        os.remove(DummyProcess_file)

    @patch('subprocess.Popen', side_effect=None)
    def test_launches_salea_application_correct_log(self, mock_subprocess):
        create_config_file(DummyProcess_file)
        process_file = os.path.abspath(DummyProcess_file)
        salea_launch_command = str(
            salea_path)+" "+"--automation --automationPort "+str(dummy_port)
