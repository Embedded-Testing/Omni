...
class TestOpenocd(unittest.TestCase):
    ...
    def test_launch_openocd_raises_exception_if_process_file_dont_exist(self):
        with self.assertRaises(FileNotFoundError):
            launch_openocd(DummyProcess_file,openocd_path,board_cfg_file_path,interface_cfg_file_path,open_ocd_log_path)
    ...
    def test_launch_openocd_dont_raise_exception_if_process_file_exist(self,mock_subprocess):
        ...
        launch_openocd(os.path.abspath(DummyProcess_file),openocd_path,board_cfg_file_path,interface_cfg_file_path,open_ocd_log_path)
    ...
    def test_launch_openocd_raises_exception_if_file_has_bad_format(self):
        ...
        with self.assertRaises(ValueError):
            launch_openocd(file_path,openocd_path,board_cfg_file_path,interface_cfg_file_path,open_ocd_log_path)
        os.remove(file_path)
    ...
    def test_launches_openocd_application_correct_call(self,mock_subprocess):
        ...
        launch_openocd(process_file,openocd_path,board_cfg_file_path,interface_cfg_file_path,open_ocd_log_path)
        mock_subprocess.assert_called_with(cmd_call.split(),stdout=ANY,stderr=ANY)
        os.remove(DummyProcess_file)
    ...
    def test_launches_openocd_creates_correct_process_entry(self,mock_append_process_data,mock_subprocess):
        ...
        assert (open_ocd_process_entry["process_call"]=="/path/to/openocd -f /board/board.cfg -f /interface/interface.cfg")
